...
/**
 * This class splits up BigDecimal plate stacks, plates, wells and well sets into 
 * bins based on the input thresholds.
 *
 * <br><br>
 * 
 ...
 */
public class Bins extends QuantileStatisticListsContext {
    
    /**
     * Calculates the median.
     * @param    List<BigDecimal>    the list
     * @param    List<BigDecimal>    the thresholds
     * @param    MathContext         the math context
     * @return                       the result
     */
    public List<List<BigDecimal>> calculate(List<BigDecimal> list, List<BigDecimal> thresholds, MathContext mc) {
    	
        Collections.sort(list);
    	
    	List<List<BigDecimal>> bins = new ArrayList<List<BigDecimal>>();
    	int max = thresholds.size();
    	...
    	for(int i = 0; i < list.size(); i++) {
    		...
    		if(index == max) {
    			bins.get(max).addAll(list.subList(i, list.size()));
    			break;
    		}
    		
    		if(list.get(i).compareTo(thresholds.get(index)) < 0) {
     ...
     * @return                   the result
     */
    public List<List<BigDecimal>>  calculate(List<BigDecimal> list, int begin, int length, List<BigDecimal> thresholds, MathContext mc) {
        return calculate(list.subList(begin, begin + length), thresholds, mc);
    }
