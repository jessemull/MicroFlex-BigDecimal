...
/**
 * This class calculates the successive differences of big decimal plate stacks, 
 ...
 */
public class Differences extends DescriptiveStatisticListWeightsContext {
    ...
    public List<BigDecimal> calculate(List<BigDecimal> list, MathContext mc) {
    	...
    	for(int i = 1; i < list.size(); i++) {
    		result.add(list.get(i).subtract(list.get(i - 1)));
    	}
     ...
     * @return                       the result
     */
    public List<BigDecimal> calculate(List<BigDecimal> list, double[] weights, MathContext mc) {
    	...
    	for(int i = 1; i < list.size(); i++) {
    		BigDecimal weighted1 = list.get(i).multiply(new BigDecimal(weights[i]));
    		BigDecimal weighted2 = list.get(i - 1).multiply(new BigDecimal(weights[i - 1]));
    		result.add(weighted1.subtract(weighted2));
    	}
     ...
     * @return                   the result
     */
    public List<BigDecimal> calculate(List<BigDecimal> list, double[] weights, int begin, int length, MathContext mc) {
