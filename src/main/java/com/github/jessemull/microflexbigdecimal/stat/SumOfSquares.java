...
/**
 * This class calculates the sum of square for BigDecimal plate stacks, plates, wells and 
 * well sets. 
 * 
 * <br><br>
 * 
 ...
 */
public class SumOfSquares extends DescriptiveStatisticWeightsContext {
    ...
    public BigDecimal calculate(List<BigDecimal> list, double[] weights, MathContext mc) {
        ...
        for(int i = 0; i < list.size(); i++) {
            sum = sum.add(list.get(i).multiply(BigDecimal.valueOf(weights[i]), mc).pow(2, mc), mc);
        }
