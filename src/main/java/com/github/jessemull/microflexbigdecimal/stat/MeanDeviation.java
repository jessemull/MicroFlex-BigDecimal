...
/**
 * This class returns the maximum value in a data set for BigDecimal plate stacks, 
 ...
 */
public class MeanDeviation extends DescriptiveStatisticContext {
	...
	public BigDecimal calculate(List<BigDecimal> list, MathContext mc) {
		...
		mean = mean.divide(new BigDecimal(list.size()), mc);
		
		BigDecimal deviation = new BigDecimal(0);
		
		for(BigDecimal value : list) {
			value = value.subtract(mean);
			value = value.abs();
			deviation = deviation.add(value);
		}
		
		return deviation.divide(new BigDecimal(list.size()));
    }
    ...
    public BigDecimal calculate(List<BigDecimal> list, int begin, int length, MathContext mc) {
        return this.calculate(list.subList(begin, begin + length), mc);            
    }
