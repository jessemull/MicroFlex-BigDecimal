...
/**
 * This class calculates the cumulative product of big decimal plate stacks, 
 ...
 */
public class CumulativeProduct extends DescriptiveStatisticListWeightsContext {
    ...
    public List<BigDecimal> calculate(List<BigDecimal> list, double[] weights, MathContext mc) {
    	...
    	for(int i = 0; i < list.size(); i++) {
    		product = product.multiply(list.get(i).multiply(new BigDecimal(weights[i])));
    		result.add(product);
    	}
