...
/**
 * This splits up big decimal plate stacks, plates, wells and well sets into 
 * sublists into bins spaced evenly between the maximum and minimum values.
 * 
 * <br>
 * 
 ...
 */
public class EqualBins extends QuantileStatisticListContext {
     ...
     * @return                       the result
     */
    public List<List<BigDecimal>> calculate(List<BigDecimal> list, int p, MathContext mc) {
    	...
    	BigDecimal binLength = (list.get(list.size() - 1).subtract(list.get(0))).divide(new BigDecimal(p), mc);
    	List<List<BigDecimal>> bins = new ArrayList<List<BigDecimal>>();

    	for(int i = 0; i < p; i++) {
    		bins.add(new ArrayList<BigDecimal>());
    	}
    	...
    	for(int i = 0; i < list.size() - 1; i++) {
    		...
    		int index = list.get(i).divide(binLength, mc).setScale(0, RoundingMode.FLOOR).intValueExact();
     ...
     * @return                       the result
     */
    public List<List<BigDecimal>> calculate(List<BigDecimal> list, int begin, int length, int p, MathContext mc) {
        return this.calculate(list.subList(begin, begin + length), p, mc);
    };
}
