...
import com.github.jessemull.microflexbigdecimal.math.Decrement;
...
/**
 * This class tests the methods in the decrement big decimal class.
 * 
 * @author Jesse L. Mull
...
public class DecrementTest {
		...
		for(Plate[] plates : arrays) {
			...
			for(Plate plate : plates) {
				...
				for(Well well : plate) {
					...
					List<BigDecimal> returned = decrement.wells(well);
        ...
        for(Plate[] plates : arrays) {
			...
			for(Plate plate : plates) {
				...
				for(Well well : plate) {
	    			...
	    			List<BigDecimal> result = new ArrayList<BigDecimal>();
					List<BigDecimal> returned = decrement.wells(well, begin, end - begin);				
					
					for(int i = begin; i < end; i++) {
						result.add(well.data().get(i).subtract(BigDecimal.ONE));
					}
        ...
        for(Plate[] plates : arrays) {
			...
			for(Plate plate : plates) {
				...
				for(Well well : plate) {
					...
					for(BigDecimal bd : well) {
						resultList.add(bd.subtract(BigDecimal.ONE));
					}
				...
				Plate returnedPlate = decrement.plates(plate);
    	...
    	for(Plate[] plates : arrays) {
			...
			for(Plate plate : plates) {
				...
				for(Well well : plate) {
					...
					for(int i = begin; i < end; i++) {
						resultList.add(well.data().get(i).subtract(BigDecimal.ONE));
					}
				...
				Plate resultPlate = new Plate(plate.rows(), plate.columns(), result);
				Plate returnedPlate = decrement.plates(plate, begin, end - begin);
    ...
    public void testSets() {
        ...
        for(Plate[] plates : arrays) {
			...
			for(Plate plate : plates) {
				...
				WellSet result = new WellSet();
				WellSet returned = decrement.sets(plate.dataSet());
    ...
    public void testSetsIndices() {
        ...
        for(Plate[] plates : arrays) {
			...
			for(Plate plate : plates) {
				...
				WellSet result = new WellSet();
				WellSet returned = decrement.sets(plate.dataSet(), begin, end - begin);
    ...
    public void testStacks() {
        ...
        for(Stack stack : stacks) {
			...
			Stack returnedStack = decrement.stacks(stack);
    ...
    public void testStacksIndices() {
    	...
    	for(Stack stack : stacks) {
			...
			Stack returnedStack = decrement.stacks(stack, begin, end - begin);
