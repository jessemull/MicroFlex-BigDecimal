...
import com.github.jessemull.microflexbigdecimal.io.PlateReader;
import com.github.jessemull.microflexbigdecimal.io.PlateWriter;
...
import com.github.jessemull.microflexbigdecimal.stat.Mean;
...
public class PlateReaderResultsTest {
	...
	private static List<Map<Well, BigDecimal>> maps = new ArrayList<Map<Well, BigDecimal>>();
	...
	public static void setUp() {
		...
		for(int j = 0; j < array.length; j++) {
			...
			labelList.add(label);
			array[j] = plate;
			maps.add((TreeMap<Well, BigDecimal>) mean.plate(plate, mc));
		}
		
		for(int k = 0; k < stackNumber; k++) {	
			Stack stack = RandomUtil.randomStackBigDecimal(
    ...
    public void testNextMap() throws IOException, JAXBException {
		...
		PlateReader reader = new PlateReader(file);
		Iterator<Map<Well, BigDecimal>> iter = maps.iterator();
		...
		while(reader.hasNextMap()) {
			...
			for(Map.Entry<Well, BigDecimal> entry : iter.next().entrySet()) {
    ...
    public void testNextJSONResult() throws IOException, JAXBException {
		...
		while(reader.hasNextJSONResult()) {
			...
			Map<Well, BigDecimal> input = iter.next();
			WellSet output = reader.nextJSONResult();
			
			Iterator<Map.Entry<Well, BigDecimal>> iterInput = input.entrySet().iterator();
			Iterator<Well> iterOutput = output.iterator();
			
			while(iterInput.hasNext()) {
				
				Map.Entry<Well, BigDecimal> entry = iterInput.next();
    ...
    public void testPreviousJSONResult() throws IOException, JAXBException {
		...
		PlateReader reader = new PlateReader(file);
		ListIterator<Map<Well, BigDecimal>> iter = maps.listIterator(maps.size());
		...
		while(reader.hasPreviousJSONResult()) {
			...
			Map<Well, BigDecimal> input = iter.previous();
